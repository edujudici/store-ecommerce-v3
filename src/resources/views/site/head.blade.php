    ...
    <div class="main_menu">
        <nav class="navbar navbar-expand-lg navbar-light main_box">
            <div class="container">
                ...
                <a class="navbar-brand logo_h" href="/"><img alt=""
                        data-bind="attr: {src: base.displayImage(company.com_image)}"></a>
                @if (auth()->user())
                    <a href="{{ route('shopper.notifications.index') }}" class="navbar-toggler pr-2"
                        title="Notificações">
                ...
                <div class="collapse navbar-collapse offset" id="navbarSupportedContent">
                    <ul class="nav navbar-nav menu_nav ml-auto">
                        <li class="nav-item" data-bind="css: {'active': navSelected() == 'home'}"><a class="nav-link"
                                href="{{ route('site.home.index') }}">Home</a></li>
                        <li class="nav-item" data-bind="css: {'active': navSelected() == 'shop'}"><a class="nav-link"
                                href="{{ route('site.shop.index') }}">Produtos</a></li>
                        <li class="nav-item" data-bind="css: {'active': navSelected() == 'faq'}"><a class="nav-link"
                                href="{{ route('site.faq.index') }}">FAQ</a></li>
                        <li class="nav-item" data-bind="css: {'active': navSelected() == 'contact'}"><a class="nav-link"
                                href="{{ route('site.contact.index') }}">Contato</a></li>
                            ...
                            <ul class="dropdown-menu">
                                @if (auth()->user())
                                    <li class="nav-item"><a class="nav-link"
                                            href="{{ route('shopper.index') }}">Início</a></li>
                                    <li class="nav-item"><a class="nav-link"
                                            href="{{ route('shopper.orders.index') }}">Pedidos</a></li>
                                    <li class="nav-item"><a class="nav-link"
                                            href="{{ route('shopper.data.index') }}">Meus Dados</a></li>
                                    <li class="nav-item"><a class="nav-link"
                                            href="{{ route('shopper.favorites.index') }}">Favoritos</a></li>
                                    <li class="nav-item"><a class="nav-link"
                                            href="{{ route('shopper.notifications.index') }}">Notificações</a></li>
                                    <li class="nav-item"><a class="nav-link"
                                            href="{{ route('shopper.vouchers.index') }}">Cupons</a></li>
                                ...
                                @else
                                    <li class="nav-item"><a class="nav-link" href="{{ route('login') }}">Entrar</a>
                                    </li>
                                @endif
                            </ul>
                        </li>
                        ...
                        <li class="nav-item nav-mobile" data-bind="visible: head.viewModel.totalItemsFavorite() > 0">
                            <a href="{{ route('shopper.favorites.index') }}" class="cart nav-link"
                                title="Favoritos">
                                <span class="ti-heart"></span>
                                <span data-bind="text: head.viewModel.totalItemsFavorite()"></span>
                            </a>
                        ...
                        @if (auth()->user())
                            <li class="nav-item submenu dropdown nav-mobile">
                                ...
                                <ul class="dropdown-menu dropdown-menu-right notification-area">
                                    <li class="dropdown-header">Você tem
                                        {{ auth()->user()->unreadNotifications->count() }} notificações</li>
                                    @foreach (auth()->user()->unreadNotifications()->take(5)->get() as $notification)
                                        @if (
                                            $notification->type == 'App\Notifications\OrderCommentNotification' ||
                                                $notification->type == 'App\Notifications\OrderNotification')
                                            <li class="nav-item">
                                                <a class="nav-link"
                                                    href="{{ route('shopper.orders.index', $notification->data['order']) }}">
                                                    <i class="ti-plus"></i> {{ $notification->data['message'] }}
                                                    <span><i class="ti-alarm-clock"></i>
                                                        {{ parseDateToPt($notification->created_at) }}</span>
                                                </a>
                                            </li>
                                        @endif
                                    @endforeach
                    ...
                    <ul class="nav navbar-nav navbar-right">
                        <li></li>
                        <li class="nav-item">
                            <button class="search" title="Pesquisar"><span class="lnr lnr-magnifier"
        ...
        self.addProductCart = function(sku, amountAux) {
            let amount = undefined === amountAux ? 1 : amountAux,
                ...
                callback = function(data) {
                    ...
                    self.totalItems(self.totalItems() + parseInt(amount));
                };
