...
class CalculaFrete
{
    ...
    public function init($_params)
    {
        ...
        if (isset($_params['nCdEmpresa'])) {
            $this->arr_params['nCdEmpresa'] = trim($_params['nCdEmpresa']);
        }
        if (isset($_params['sDsSenha'])) {
            $this->arr_params['sDsSenha'] = trim($_params['sDsSenha']);
        }
        if (isset($_params['sCepOrigem'])) {
            $this->arr_params['sCepOrigem'] = trim($_params['sCepOrigem']);
        }
        if (isset($_params['sCepDestino'])) {
            $this->arr_params['sCepDestino'] = trim($_params['sCepDestino']);
        }
        if (isset($_params['nVlPeso'])) {
            $this->arr_params['nVlPeso'] = trim($_params['nVlPeso']);
        }
        if (isset($_params['nVlComprimento'])) {
            $this->arr_params['nVlComprimento'] = trim($_params['nVlComprimento']);
        }
        if (isset($_params['nVlAltura'])) {
            $this->arr_params['nVlAltura'] = trim($_params['nVlAltura']);
        }
        if (isset($_params['nVlLargura'])) {
            $this->arr_params['nVlLargura'] = trim($_params['nVlLargura']);
        }
        if (isset($_params['nVlValorDeclarado'])) {
            $this->arr_params['nVlValorDeclarado'] = trim($_params['nVlValorDeclarado']);
        }
        if (isset($_params['nCdServico'])) {
            $this->arr_params['nCdServico'] = trim($_params['nCdServico']);
        }
        if (isset($_params['url'])) {
            $this->arr_params['url'] = trim($_params['url']);
        }
        if (isset($_params['nCdFormato'])) {
            $this->arr_params['nCdFormato'] = trim($_params['nCdFormato']);
        }
        if (isset($_params['sCdMaoPropria'])) {
            $this->arr_params['sCdMaoPropria'] = trim($_params['sCdMaoPropria']);
        }
        if (isset($_params['sCdAvisoRecebimento'])) {
            $this->arr_params['sCdAvisoRecebimento'] = trim($_params['sCdAvisoRecebimento']);
        }
        if (isset($_params['nVlDiametro'])) {
            $this->arr_params['nVlDiametro'] = trim($_params['nVlDiametro']);
        }
        if (isset($_params['StrRetorno'])) {
            $this->arr_params['StrRetorno'] = trim($_params['StrRetorno']);
        }
        if (isset($_params['nIndicaCalculo'])) {
            $this->arr_params['nIndicaCalculo'] = trim($_params['nIndicaCalculo']);
        }
    }
    ...
    public function request($args = null): array
    {
        ...
        try {
            ...
            $description = ServicosCorreios::getDescription(
                $this->arr_params['nCdServico']
            );
            $xml = '
                <ws:example xmlns:ws="http://example.com">
                    <Erro>1</Erro>
                    <MsgErro>Ocorreu um erro ao consultar o serviço: '
                        . $description .'
                    </MsgErro>
                </ws:example>';
            return new SimpleXMLElement($xml, 0, false);
        }
    }
}
